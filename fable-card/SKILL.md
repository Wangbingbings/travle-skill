---
name: fable-card
description: "Content caster (铸). Transforms content into PNG visuals. Two molds: -l (default) long reading card, -i infograph (1080xauto). Output to ~/Downloads/. Use when user says '铸', 'cast', '做成图', '做成卡片', '做成信息图', '做成海报'. Replaces fable-cards and fable-infograph."
user_invocable: true
version: "2.4.0"
---
...
- 内容来源（可选）——有明确来源时显示（如作者名、arxiv ID、网站名等），无来源时留空。使用 `{{SOURCE_LINE}}` 变量：有来源时填 `<span class="info-source">来源文字</span>`，否则空字符串。
